...
- **Messages de contact/reservation predefinis** (WhatsApp, SMS/iMessage, email envoyes au proprietaire) : en **francais pour `fr.json`** et en **anglais pour toutes les autres langues** (`en.json`, `nl.json`, `de.json`). Cles concernees : `contact_whatsapp_message`, `contact_sms_message`, `tarifs_book_msg`.
- **Messages de partage predefinis** (partage vers proches de l'utilisateur) : restent dans la **langue d'origine** de chaque fichier. Cles concernees : `share_text`, `share_email_subject`, `share_email_body`.
- **Donnees structurees (JSON-LD)** : ne jamais inventer ou deviner des URLs dans le JSON-LD. Si une URL semble obsolete ou inconnue, demander confirmation a l'utilisateur avant de l'inclure.